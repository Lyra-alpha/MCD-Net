...
if __name__ == "__main__":
    ...
    if miou_mode == 0 or miou_mode == 1:
        ...
        print("Load model.")
        deeplab = MCDNetPredictor(use_attention=use_attention)
        print("Load model done.")

        print("Get predict result.")
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path,"Moraine_dataset/JPEGImages/"+image_id+".jpg")
    ...
    if miou_mode == 0 or miou_mode == 2:
        print("Get miou.")
        hist, IoUs, PA_Recall, Precision, F1_scores = compute_mIoU(gt_dir, pred_dir, image_ids, num_classes, name_classes)  # 执行计算mIoU的函数
        print("Get miou done.")
        show_results(miou_out_path, hist, IoUs, PA_Recall, Precision, F1_scores, name_classes)
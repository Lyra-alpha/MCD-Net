...
class MCDNetPredictor(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.num_classes <= 21:
            self.colors = [ (0, 0, 0), (0, 128, 0) ]  # 背景和冰碛物
        else:
            import colorsys
    ...
    def generate(self, onnx=False):
        ...
        try:
            ...
            # 检查权重文件是否包含CBAM权重
            has_cbam_weights = any('backbone_attention' in key for key in state_dict.keys())
            print(f"权重文件包含CBAM权重: {has_cbam_weights}")
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        with torch.no_grad():
            ...
            #   取出每一个像素点的种类
            #---------------------------------------------------#
            pr = pr.permute(0, 2, 3, 1)  # [batch, H, W, C]
